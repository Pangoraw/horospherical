...
def prototype_unify(num_classes):
    ...
    set_prototypes = set_prototypes[torch.randperm(set_prototypes.size(0))]
...
if __name__ == "__main__":
    ...
    path = os.path.join(args.resdir, "prototypesuniform-%dd-%dc.npy" % (args.dims, args.classes))
    np.save(path, prototypes.data.cpu().numpy())
    print(f">> save prototypes to {path}")
